  ...
  const confirmAddAllAttendees = async () => {
    ...
    const newResult = await Promise.all(
      projectMembers.map(async (member) => {
        const startTimeCopy = newRoomBooking.startTime.split(':')
        const endTimeCopy = newRoomBooking.endTime.split(':')
        const prepareObj = {
          attendeeId: member.id,
          attendeeName: member.fullName,
          startTime: `${newRoomBooking.fromDate}/${startTimeCopy[0]}/${startTimeCopy[1]}`,
          endTime: `${newRoomBooking.fromDate}/${endTimeCopy[0]}/${endTimeCopy[1]}`,
        }
