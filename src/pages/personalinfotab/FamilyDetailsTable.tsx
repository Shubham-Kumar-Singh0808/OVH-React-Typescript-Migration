...
const FamilyDetailsTable:React.FC<FamilyInfo> = ({ isFieldDisabled = false,
  striped=true,bordered=true,tableClassName = '',}:FamilyInfo): JSX.Element => {
...
const tableDataCellProps = {
  ...
  return (
      <>
      <CTable responsive striped={striped} bordered={bordered} className={tableClassName}>
      {!isFieldDisabled ?(
        <>
    ...
    {familyDetails.map((family, index) => (
                <CTableRow key={index}>
                  ...
                  <CTableDataCell scope="row">{family.contactNumber || 'N/A'}</CTableDataCell>
                  <CTableDataCell scope="row">{family.dateOfBirth || 'N/A'}</CTableDataCell>   
                  {isFieldDisabled ? (             
  ...
  {isFieldDisabled && (
    <>
      <strong>
      {familyDetails.length ? `Total Records: ${familyDetails.length}` : `No Records found`}
      </strong>
    </>
  )}
  </>
